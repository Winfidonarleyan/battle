...
void AnticheatMgr::BuildReport(Player* player, uint8 reportType)
{
	...
	if (m_Players[key].GetTotalReports() > sWorld->getIntConfig(CONFIG_ANTICHEAT_REPORTS_INGAME_NOTIFICATION) && next == 0)
	{
		std::string type;
		switch (reportType)
		{
		...
		sWorld->SendGMText(LANG_ANTICHEAT_ANNOUNCE, player->GetName().c_str(), type.c_str());
		next = 60;
