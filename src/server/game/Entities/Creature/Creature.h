...
class Creature : public Unit, public GridObject<Creature>, public MapObject
{
    public:
		...
		void SetSpell(uint8 i, uint32 spell) { if (i > CREATURE_MAX_SPELLS - 1) i = CREATURE_MAX_SPELLS - 1; m_spells[i] = spell; }
		void ClearAllSpells() { for (uint8 i = 0; i < CREATURE_MAX_SPELLS; ++i) m_spells[i] = 0; }
