...
bool IsSartarionInCombat(Unit* me, InstanceScript* instance)
{
    if (Creature* sart = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_SARTHARION)))
        if (sart->IsAlive() && sart->IsInCombat())
            return true;
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                switch (eventId)
                {
                    case EVENT_ACOLYTE_VESPERON:
                        ...
                        {
                            if (IsSartarionInCombat(me, instance)) DoCastVictim(SPELL_TWILIGHT_TORMENT_VESP);
                ...
                if (map->IsDungeon())
                {
                    ...
                    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                    {
                        if (i->GetSource()->IsAlive() && i->GetSource()->HasAura(SPELL_TWILIGHT_SHIFT) && !i->GetSource()->GetVictim())
                        {
                            ...
                            if (IsSartarionInCombat(me, instance)) i->GetSource()->CastSpell(i->GetSource(), SPELL_TWILIGHT_RESIDUE, true);
    ...
    public:
        ...
        struct npc_acolyte_of_vesperonAI : public ScriptedAI
        {
            ...
            void Reset() override
            {
                ...
                if (IsSartarionInCombat(me, instance)) DoCast(me, SPELL_TWILIGHT_TORMENT_VESP_ACO);
            }
