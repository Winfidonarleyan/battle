...
enum Falalala_Ogrila_Ids
{
    SPELL_FLYING_REINDEER_310                   = 44827,
    SPELL_FLYING_REINDEER_280                   = 44825,
    SPELL_FLYING_REINDEER_60                    = 44824,
    QUEST_BOMB_THEM_AGAIN                       = 11023,
    ACHIEVEMENT_FALALALA_OGRILA                 = 1282,
};

class achievement_Falalala_Ogrila : public PlayerScript
{
    public:
        achievement_Falalala_Ogrila() : PlayerScript("achievement_Falalala_Ogrila") {}

        void OnQuestStatusChange(Player* player, uint32 questId, QuestStatus status)
        {
            if (questId == QUEST_BOMB_THEM_AGAIN && status == QUEST_STATUS_COMPLETE && IsHolidayActive(HOLIDAY_FEAST_OF_WINTER_VEIL)
            && (player->HasAura(SPELL_FLYING_REINDEER_310) || player->HasAura(SPELL_FLYING_REINDEER_280) || player->HasAura(SPELL_FLYING_REINDEER_60)))
            {
                const AchievementEntry* achiev = sAchievementStore.LookupEntry(ACHIEVEMENT_FALALALA_OGRILA);
...
void AddSC_achievement_scripts()
{
    ...
    new achievement_Falalala_Ogrila();
}
