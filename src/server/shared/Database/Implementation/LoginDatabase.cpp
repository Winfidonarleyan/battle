...
void LoginDatabaseConnection::DoPrepareStatements()
{
	...
	PrepareStatement(LOGIN_SET_EXPIRED_ACC_PREM, "UPDATE account_premium SET active = 0 WHERE unsetdate<=UNIX_TIMESTAMP() AND unsetdate<>setdate", CONNECTION_ASYNC);
	PrepareStatement(LOGIN_SEL_ACC_PREM, "SELECT 1 FROM account_premium WHERE id = ? AND active = 1", CONNECTION_SYNCH);
}
